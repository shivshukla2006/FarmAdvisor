  ...
  return (
    <group ref={groupRef} position={[0, -0.3, 0]}>
      ...
      <mesh position={[0, 0.6, 0]}>
        ...
        <meshStandardMaterial color="hsl(120, 61%, 34%)" roughness={0.4} metalness={0.3} />
      </mesh>

      {/* Cabin */}
      <mesh position={[-0.2, 1.2, 0]}>
        <boxGeometry args={[1, 0.8, 0.9]} />
        <meshStandardMaterial color="hsl(120, 61%, 28%)" roughness={0.3} metalness={0.4} />
      </mesh>
      ...
      <mesh position={[0.9, 0.45, 0]}>
        ...
        <meshStandardMaterial color="hsl(120, 61%, 38%)" roughness={0.4} metalness={0.3} />
      </mesh>
      ...
      {/* Wheel hubs */}
      {[[-0.5, 0.05, 0.75], [-0.5, 0.05, -0.75], [0.8, 0.05, 0.6], [0.8, 0.05, -0.6]].map((pos, i) => (
        <mesh key={i} position={pos as [number, number, number]} rotation={[0, Math.PI / 2, 0]}>
      ...
      {/* Steering wheel */}
      <mesh position={[0.15, 1.15, 0]} rotation={[0.3, 0, 0]}>
      ...
      <mesh position={[-0.5, 0.45, 0.62]}>
        ...
        <meshStandardMaterial color="hsl(120, 61%, 30%)" roughness={0.4} metalness={0.3} />
      </mesh>
      <mesh position={[-0.5, 0.45, -0.62]}>
        <boxGeometry args={[0.6, 0.06, 0.3]} />
        <meshStandardMaterial color="hsl(120, 61%, 30%)" roughness={0.4} metalness={0.3} />
...
const GrassGround = () => (
  <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.75, 0]} receiveShadow>
    ...
    <meshStandardMaterial color="hsl(120, 40%, 30%)" roughness={0.9} />
  </mesh>
);
  ...
  useFrame((state) => {
    if (leaves.current) {
      leaves.current.children.forEach((leaf, i) => {
        ...
        leaf.rotation.x = Math.sin(state.clock.elapsedTime * 0.4 + i) * 0.5;
      });
    }
  });
    ...
    <group ref={leaves}>
      {[0, 1, 2, 3].map((i) => (
        <mesh key={i} position={[Math.cos(i * 1.5) * 1.8, 1.5, Math.sin(i * 1.5) * 1.8]}>
          ...
          <meshStandardMaterial color="hsl(120, 50%, 45%)" side={THREE.DoubleSide} transparent opacity={0.8} />
        </mesh>
      ))}
    </group>
  );
